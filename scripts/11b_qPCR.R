...
#Import dataset
Synthesis <- read_csv("./data/11_qPCR/input/11b_SynthesisMNE.csv", show_col_types = FALSE)
...
view(Synthesis_summary)

write_csv(Synthesis_summary,"./data/11_qPCR/output/11b_SynthesisMNE_summary.csv")
...
pdf("./figures/11b_qPCR_time0_treatment_all_data.pdf", width = 12, height = 8)
for (p in plots_treatments) {
  if (!is.null(p)) print(p)
}
dev.off()

pdf("./figures/11b_qPCR_mock_timepoints_all_data.pdf", width = 12, height = 8)
...
pdf("./figures/11b_qPCR_targets_x_paper.pdf", width = 12, height = 8)
for (p in plots) {
  if (!is.null(p)) print(p)
}
dev.off()
