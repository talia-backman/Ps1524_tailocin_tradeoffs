...
all_exp <- read.table("./data/input/tnseq_plant.csv", header = TRUE, sep=",")
...
orth_map <- read_tsv("./data/input/tnseq_ortho.csv",show_col_types = FALSE) %>%
  transmute(dc3000_id = name_dc3000_new, p25_id = name_p25c2) %>% distinct()
# prepare the mapping and rename its p25 column to avoid suffix confusion
orth_map2 <- orth_map %>%
  dplyr::rename(p25_id_map = p25_id)
# join and coalesce (no keep=TRUE; keeps a single dc3000_id)
...
tailocin <- read.csv("./data/input/tnseq_tailocin.csv", row.names = 1)
...
# export data
write.csv(merged_tradeoffs_all,"./data/output/tnseq_merged.csv")
