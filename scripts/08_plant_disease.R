...
# note: manually re-named the split plate (Col0-Eyach_R3) to match contam data frame
...
layout_plate <- read.csv("./data/input/plant_infections_layout.csv", header = T)
...
full_plot_data <- merged_data %>%
  dplyr::count(strain, Phenotype, ecotype) %>%
...
p
# save as pdf
ggsave("./figures/plant_disease.pdf", p, width = 7, height = 4)
